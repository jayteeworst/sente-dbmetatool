...
namespace DbMetaTool
{
    public static class Program
    {
        ...
        public static void BuildDatabase(string databaseDirectory, string scriptsDirectory)
        {
            ...
            string dbPath = Path.Combine(databaseDirectory, "megadb.fdb");

            if (File.Exists(dbPath))
            {
                Console.WriteLine($"Usuwanie istniejącej bazy danych: {dbPath}");
                File.Delete(dbPath);
            }

            string createConnStr = $"User=SYSDBA;Password=masterkey;Database={dbPath};ServerType=1;Charset=UTF8";

            Console.WriteLine($"Tworzenie bazy danych: {dbPath}");
            FbConnection.CreateDatabase(createConnStr, overwrite: false);

            using var connection = new FbConnection(createConnStr);
            connection.Open();

            var executor = new ScriptExecutor(connection);

            Console.WriteLine("Wykonuję skrypty domen...");
            executor.ExecuteScriptsFromDirectory(scriptsDirectory, "domains");

            Console.WriteLine("Wykonuję skrypty tabel...");
            executor.ExecuteScriptsFromDirectory(scriptsDirectory, "tables");

            Console.WriteLine("Wykonuję skrypty procedur...");
            executor.ExecuteScriptsFromDirectory(scriptsDirectory, "procedures");
        }
        ...
        public static void ExportScripts(string connectionString, string outputDirectory)
        {
            ...
            var exporter = new MetadataExporter(connection, outputDirectory);

            Console.WriteLine("Eksportuję domeny...");
            exporter.ExportDomains();

            Console.WriteLine("Eksportuję tabele...");
            exporter.ExportTables();

            Console.WriteLine("Eksportuję procedury...");
            exporter.ExportProcedures();
        }
        ...
        public static void UpdateDatabase(string connectionString, string scriptsDirectory)
        {
            ...
            Console.WriteLine("Aktualizuję domeny...");
            executor.ExecuteScriptsFromDirectory(scriptsDirectory, "domains");

            Console.WriteLine("Aktualizuję tabele...");
            executor.ExecuteScriptsFromDirectory(scriptsDirectory, "tables");

            Console.WriteLine("Aktualizuję procedury...");
            executor.ExecuteScriptsFromDirectory(scriptsDirectory, "procedures");
        }
    }
}
